...
<div class="form-group{{ $errors->has('session_id') ? ' has-error' : '' }}">
    ...
    <div class="col-md-6">
        <select multiple="multiple" name="session_id[]" id="sports" class="form-control">
        @foreach($sessions as $session_key => $session_when)
            <option value="{{$session_key}}"@if (in_array("$session_key", $booked_session, true))selected="selected"@endif>{{$session_when}}</option>
        @endforeach
        </select>
    </div>
</div>
